...
public class Ex01 extends HttpServlet {
    ...
    public Ex01() {
        super();
        //  System.out.println(); 은 콘솔창에 출력된다. (웹 페이지에는 출력되지 않는다)
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 서비스의 역할은 클아이언트가 정보를 요청할때 요청 방식에 따라 알맞는 메소드를 호출하는 역할을 한다.
		// 요청 방식 : - get 방식 이면 doGet() 이동
		//			   - post 방식 이면 doPost() 이동
		// ** get방식(생략가능) : 정보를 http 패킷의 head 에 담아서 보낸다.
		//						  주소창에 해당 주소가 보인다.
		//						  속도가 빠르고, 적은 양에 적합, 보안에 취약
		// ** post방식 : 정보를 http 패킨의 body에 담아서 보낸다.
		//				 주소창에 해당 정보가 보이지 않는다.
		// 				 속도는 get방식보다 느리고, 많은 양의 정보를 전달할 때 적합하고, get방식보다 보안적이다.
		
		// ** request : 클라이언트에게 들어온 요청 정보를 가지고 있는 객체 (요청)
		// ** response : 클라이언트에게 결과를 보여줄 수 있는 응답 정보를 가지고 있는 객체 (응답)
	...
	public void destroy() {
		// 해당 프로젝트가 톰캣에서 삭제되기 직전에 실행하는 메소드
